...
// Fix: Cast window to `any` to avoid TypeScript errors for browser-specific APIs.
...
const Chatbot: React.FC = () => {
    ...
    const [messages, setMessages] = useState<ChatMessage[]>([
        { id: 1, text: "Namaste! I am your Darshan Sahayak. How may I assist you today? You can ask about crowd status, temple timings, or even book a darshan.", sender: 'bot' }
    ]);
    ...
    const handleVoiceInput = () => {
        ...
        recognition.onend = () => {
            setIsListening(false);
            // Automatically send after speech ends.
            // Using a timeout to allow state to update before sending.
            setTimeout(() => {
                 handleSendMessage();
            }, 100);
        };
    ...
    return (
        <>
            ...
            {isOpen && (
                <div className="fixed bottom-24 right-6 w-[90vw] max-w-sm h-[70vh] max-h-[600px] bg-cream rounded-2xl shadow-2xl flex flex-col z-50 overflow-hidden border-2 border-amber-light">
                    <header className="bg-gradient-to-r from-amber to-orange-light p-4 text-white flex justify-between items-center">
                        <h3 className="font-bold text-lg">Darshan Sahayak</h3>
                    </header>
                    <div ref={chatboxRef} className="flex-1 p-4 overflow-y-auto space-y-4">
                        {messages.map((msg) => (
                             <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className={`max-w-[80%] p-3 rounded-2xl ${msg.sender === 'user' ? 'bg-amber-light text-warmGray-900 rounded-br-none' : 'bg-white text-warmGray-900 rounded-bl-none shadow-sm'}`}>
                                    <p className="text-sm" dangerouslySetInnerHTML={{ __html: msg.text.replace(/\n/g, '<br />') }} />
                                </div>
                                {msg.sender === 'user' && <div className="w-8 h-8 rounded-full bg-warmGray-300 flex-shrink-0 flex items-center justify-center"><UserIcon className="w-5 h-5 text-warmGray-700" /></div>}
                            </div>
                        ))}
                         {isLoading && (
                            <div className="flex items-end gap-2 justify-start">
                                ...
                                <div className="max-w-[80%] p-3 rounded-2xl bg-white text-warmGray-900 rounded-bl-none shadow-sm flex items-center space-x-2">
                    ...
                    <div className="p-4 border-t border-amber-light/30 bg-white">
                        <div className="flex items-center bg-warmGray-100 rounded-full p-1">
                             <input
                                type="text"
                                value={inputValue}
                                ...
                                className="flex-1 bg-transparent px-4 py-2 text-sm text-warmGray-700 focus:outline-none"
                                disabled={isLoading}
                            />
